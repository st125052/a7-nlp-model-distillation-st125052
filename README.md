...
A `label_list` is created, which contains all the unique labels used in the dataset `("noHate", "hate", "idk/skip", "relation")`. Then, a dictionary called `label2id` is created that assigns each label a unique numerical ID. For instance, "not hate speech" is mapped to 0, "hate speech" to 1, and so on. The next line reverses this mapping to create id2label, which converts back from a numerical prediction to a human-readable label.
